...
class PendingImportResultStore @Inject constructor(
    ...
    fun snapshot(): PendingImportResultSnapshot {
        val tokens = runCatching {
            val array = JSONArray(preferences.getString(KEY_TOKENS, "[]"))
        ...
        return PendingImportResultSnapshot(tokens, focusedCardId, notice)
    }
    ...
    @Synchronized
    fun complete(focusedCardId: String?, notice: ImportedPhotoResultNotice?) {
        preferences.edit()
    ...
    @Synchronized
    fun clearNotice() {
        preferences.edit().remove(KEY_NOTICE).commit()
    }
