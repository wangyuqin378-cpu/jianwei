...
interface PhotoDao {
    ...
    @Query("SELECT * FROM photo_candidates WHERE localId = :localId AND analysisState = 'READY' AND (:includeMediaStore = 1 OR origin != 'MEDIA_STORE') AND ((:originScope = 'MEDIA_STORE' AND origin = 'MEDIA_STORE') OR (:originScope = 'EXPLICIT_IMPORT' AND origin != 'MEDIA_STORE')) LIMIT 1")
    suspend fun eligibleCandidateForAnalysis(localId: Long, includeMediaStore: Int, originScope: String): PhotoCandidateEntity?
