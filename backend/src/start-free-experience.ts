...
async function main(args: string[]): Promise<void> {
  ...
  if (parsed.selfTest) {
    ...
    assert.equal(environment.MAX_JOBS_GLOBAL_PER_MONTH, "12");
    assert.equal(environment.OBJECT_STORE, "local");
    assert.equal(environment.DATABASE_URL, undefined);
    process.stdout.write(
      `FREE_EXPERIENCE_LAUNCHER_SELF_TEST=GO model=${FREE_EXPERIENCE_MODEL} cloudInfrastructure=0 monthlyJobs=12\n`
    );
    return;
  }
  ...
  process.stdout.write(
    `FREE_EXPERIENCE_SERVER_START model=${FREE_EXPERIENCE_MODEL} cloudInfrastructure=0 monthlyJobs=12\n`
  );
