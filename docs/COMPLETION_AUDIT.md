...
2026-07-31 Beta.73 正式安装品牌与最终构建（已由当前首次体验候选继承）：Android 已使用暖白、深绿和锈红的 adaptive/round Launcher 图标，Android 13 monochrome 图标在 Pixel Launcher 主题模式实际渲染通过；物品提醒使用独立白色单色通知图标，私有和公开通知均由设备测试锁定。API 31 主题显式声明 SplashScreen 背景、动画前景和图标背景，最终 Debug APK 资源表核验三项属性已经打包。设置导航回归暴露旧测试只能向下滚动，现改为双向搜索并完成专项及 App 30/30 重验。其 Android 字节与候选已由上述当前候选重新构建和绑定。

2026-07-30 Beta.73 真实照片每日卡与可读事实小组件（已由当前安装品牌候选继承）：App 每日卡和 2×2/4×2 Glance 组件均使用仓库内无人物扫帚照片的真实本地文件 URI，绑定已审核 `broom-001` 与 Google Patents 来源；每日卡场景继续验证识别把握、推送理由、组件入口、收藏、物品提醒和四类反馈，组件场景验证真实 Pin、缩放和来源入口。1.6× 字号视觉复核发现列表底部 8dp 空间不足以把最终反馈区滚到视口主体，现增为 96dp 并由标准/1.6× 专项复验。最终 App 30/30、Data 90/90、Android JVM 278/278，Gradle 聚合 311 个任务；其 Android 字节与旧候选已由上述当前候选取代。

2026-07-30 Beta.73 审核事实标题与卡片去重（已由当前真实照片候选继承）：每日卡最高层级从通用“关于某物”模板改为已通过 Qwen + `cip` 审核事实的首个完整短句；不合格首句继续回退安全模板，低置信度继续明确不确定性，全程没有第二次模型调用或目录外知识生成。App 和 Glance 组件去掉正文中与标题完全相同的前缀，详情标题启用中文 Heading 均衡换行；自动卡片审核升级为 `derived-ai-reviewed-card-v2`。标准/1.6× API 34 专项各 1/1，Data 90/90、App 29/29，最终 post-fix Gradle 284 个任务；后端 133/133、TypeScript check/build、源码/API/供应链/卡审计/Beta 证据门禁均通过。其当时候选已由当前真实照片候选取代。

2026-07-30 Beta.73 首次体验选择公平性（已由当前候选继承）：首次体验第 3 屏不再先展示一张很长的自动发现授权卡，而是先同屏展示“自动发现”和“仅选择照片”两张 RadioButton 选择卡；用户选定开始方式后再处理兴趣和自动准备模式。仅自动发现展开“提前备好一周 / 当天只理解一张”，单一主按钮随选择切换为“开启自动发现 / 选择一张照片”。开始方式、兴趣和自动准备模式在 Activity 重建后保持。API 34 专项 2/2、Data 设备 90/90、App 设备 29/29；其 Android 能力已由当前审核事实标题候选完成再构建与复验。

以下 PostgreSQL 与容器安全段落保留其完成时的证据措辞；其中旧候选已由上述首次体验候选重新绑定并取代。

2026-07-30 Beta.73 PostgreSQL 与容器安全本地闭环（当前最新工程摘要）：本机已安装并验证 Serverless Devs 3.1.10、PostgreSQL 17.10、Docker 29.6.2、Colima 0.10.3、Buildx 0.35.0 与 Trivy 0.72.0。ARM VM 内的 amd64 用户态模拟在 Node/pnpm 收尾处稳定触发 libuv 断言，因此建立独立 x86_64 VM 完成原生生产架构构建。Dockerfile 将完整构建与 `--prod --no-optional` 运行依赖拆层，并从 runtime 移除 npm/corepack；基础镜像固定为 Node 22.23.1 trixie-slim index digest `sha256:e6d9a389d34ff9678438af985c9913fbd1eb6ed36e80fea56644f4b4f6dd70ba`。当前镜像 `jianwei-api:beta73-production-security-v2` 为 `linux/amd64`、86,601,643 字节、默认用户 `node`，OCI image ID `sha256:6fde2adc644814df506da660c25a72f584f48078fe968d8d39da0c41aa7a8cd4`；Node 22.23.1，npm/corepack 不存在，live/ready 成功并返回后端 Release `aa912619b16e71c941dfd855a8e783cb84831115ef20d8a90618a8d357cc15f1`。PostgreSQL 17.10 已对 15 个迁移执行三轮并通过 17 项真实集成测试与 TCP E2E。升级 Fastify 并锁定传递依赖后，可修复 HIGH/CRITICAL 从 3 项降为 0；Trivy 仍报告 22 项当前无修复版本的基础系统 HIGH/CRITICAL，因此不宣称“零漏洞”。CycloneDX SBOM 含 232 个组件，失败关闭镜像证据 SHA-256 为 `95118eb073785d4478114dad3de0ccbbfe8f9f2d24c1836cff0fc35fb45b2bce`。当前跨工件候选 `.tooling/release-candidate/beta73-image-security-bound.json` 把源码/APK、镜像 ID、完整报告、SBOM 与派生指标绑定并复验为 `LOCAL_CANDIDATE_GO`，SHA-256 `ed43b743f0cdc29114e04ee7830149867fa7ed732390fcbc92278f769bc1d838`；15 类安全证据伪造和 12 类候选伪造均被自测拒绝。固定良性文本又复核北京工作空间的 Qwen `cip` 护栏为 HTTP 200 / `guardrailAccess=GO`，没有再次发送图片；旧 403 已解除。云预检虽已识别 `s/psql/docker=1`，但 RAM access、FC/RDS/OSS/VPC/ACR/HTTPS 与生产变量仍缺；未创建云资源或外发照片，全部本地证据均为 `releaseEvidence=false`，整体 Beta 保持 `NO_GO`。

2026-07-30 Beta.73 ARM 内测分发体积收敛（当前最新产品摘要）：APK 内容审计发现 Beta.72 unsigned Release 为 128,833,539 字节，主要原因是捆绑版 ML Kit 同时打入 x86、x86_64、arm64-v8a、armeabi-v7a 四套原生库；前两套只服务模拟器，却会让受控真机内测多下载约 67 MB。现在 Release 精确保留 `arm64-v8a + armeabi-v7a`，Debug 继续支持模拟器，版本推进为 `73 / 0.1.0-beta73`。最终 unsigned Release 为 61,552,852 字节，减少 52.2%；APK 实际 ABI 已从中央目录复核。候选装配器现直接解析 APK，ABI 多一套、少一套或体积超过 70 MiB 均 `NO_GO`，自测 8 类绕过全部拒绝。Android 14 设置页路径 1/1 与最终 Gradle 聚合 311 个任务通过。Debug / unsigned Release / App 测试 APK SHA-256 为 `465dae87b1126ae22261d2792dc228c9cd2f04ed2351669e17796aa695ec4948` / `796f75284fd4d19126d3b340d101a2676539a717de2379e1ce2fe083ea025284` / `7da53bfb993387f85dc12ea2063196c5ffa0306bf335bf3a0264d0d9c0512153`；候选 `.tooling/release-candidate/beta73-arm-beta-distribution-final.json` 复验为 `LOCAL_CANDIDATE_GO`，SHA-256 `3e64f2ce8df4e93d4427bcfb2d72e98f735d8e2153829e6edca123d088a76b5c`。本轮未调用云服务、Provider 或外发照片；未签名、真实云、OEM/真人无障碍与 cohort 等外部证据仍使整体 Beta 保持 `NO_GO`。

2026-07-30 Beta.72 可核验的内测构建身份（当前最新产品摘要）：发布审查发现，候选已经推进到 Beta.71，但 APK 仍使用 `versionCode=1 / versionName=0.1.0-beta01`，真实分发后无法可靠区分版本，也会破坏后续覆盖安装和 cohort 证据绑定。现在 Debug/Release 统一为 72 / `0.1.0-beta72`，设置页持续显示“见微 0.1.0-beta72 · 内测版”；候选装配器和源码门禁同时拒绝 code/name 错配。Android 14 设置页路径 1/1 通过；最终 Gradle 聚合 311 个任务，JVM 测试、Debug/Release Lint、Debug/App 测试/R8 unsigned Release APK 均通过，输出元数据也确认两个变体身份一致。Debug / unsigned Release / App 测试 APK SHA-256 为 `aff8b83aecadb1219d64de924a2f49e5d6acef44a5fe28663723477c6c71f0c9` / `48b6d17c9b8568c23a5c7bdb8efa2ce075b52613c2c218c753cc8a276860a7d9` / `8f215d35d95c3c0b1e20bab67df8a541b3b7294f6fd6eb05cdde5a62938a60a5`；候选 `.tooling/release-candidate/beta72-versioned-pilot-final.json` 为 `LOCAL_CANDIDATE_GO`、`releaseEvidence=false`，复验通过，SHA-256 `497cc94fd0f0872f17acccdf62d25aa3db0a46c67476d2f10ab050f569195c2c`。本轮没有云请求、Provider 请求或照片外发；`evaluation/beta-evidence.json`、真实云部署与回执、正式签名、授权图片评测、OEM、真人无障碍、生产卡复算与 cohort 仍缺，整体继续 `NO_GO`。

2026-07-29 Beta.71 自动发现可撤销与后台停机屏障（当前最新产品摘要）：点击路径与生命周期审查发现，界面上的“暂停”不足以证明已经运行的扫描、隐私筛选和上传链会同时停住，自动发现是否启用也需要独立于照片权限长期保存。现在自动发现默认关闭且必须明确开启；仅选择照片、设置关闭、权限撤销和删除数据都进入同一会话屏障，Worker 在整段执行和逐候选处重复核验。重复拒绝会转到应用设置，Android 14 部分照片继续受支持；组件换卡不会因数据库重排重复展示，未来卡提前换到今天后不会在原计划日重现；提醒通知不可用会重试而不是假成功。最终 Gradle 聚合 307 个任务通过；Android JVM 271/271（76/111/84）、Data 设备 90/90、App 29 项隔离运行全部通过，后端 133/133、TypeScript、源码、API、知识和候选门禁通过。Debug / unsigned Release APK SHA-256 为 `2259013354bb1df6bb344220fc5c3ac111c8b0f520d14846b09666d437b746a6` / `445962a75afdeccbcb049ac5824def2a1d425b329331db47083031c5a2112048`；候选 `.tooling/release-candidate/beta71-automatic-discovery-control.json` 为 `LOCAL_CANDIDATE_GO`、`releaseEvidence=false`，SHA-256 `6070629cb7fd602a2ac717a18e4cc19a18dc25a09e0a265edd098565775473b3`。本轮没有云请求、Provider 请求或照片外发；`evaluation/beta-evidence.json`、真实云、正式签名、授权图片评测、OEM、真人无障碍、生产卡复算与 cohort 仍缺，整体继续 `NO_GO`。

2026-07-29 Beta.70 反馈后的对象纠错与精确回滚（当前最新产品摘要）：完整点击路径审查发现，用户点过“有意思 / 没意思”后旧界面会隐藏“识错了 / 太私人”，使误识别和隐私纠正不可达。现在两项终态纠正会继续显示；确认识错会归档卡片、取消收藏、清理旧 LIKE/SAVE outbox、安排提醒删除并撤销学习权重。全量回归进一步发现，主题权重在 2.0 上限饱和时按理论增量回滚会过度扣减；Room 15、PostgreSQL 迁移 015 与两套仓储现记录并扣除每次反馈实际应用的贡献，1.35 → 2.0 可精确恢复 1.35。标准与 1.6× API 34 点击路径通过；Data 89/89，App 全部 29 项在干净模拟器三个隔离组 11+1+17 中通过，单进程运行因 WorkManager/runner 状态污染不作为 29/29 证据。Android JVM 260/260（75/109/76）、后端 133/133、TypeScript、双模块双变体 Lint、Debug/App 测试 APK/R8 unsigned Release、源码门禁、候选复验和差异检查均通过。审计 `.tooling/beta70-post-feedback-object-correction/audit.json` SHA-256 `a3d2edf5ad5e6765ba3e354596469c781cbaaacf76a83d973d22b5404d1d5d22`；候选 `.tooling/release-candidate/beta70-post-feedback-object-correction.json` 为 `LOCAL_CANDIDATE_GO`、`releaseEvidence=false`，SHA-256 `95b6b98f32ae99e17baef6014942d6a4c62b76489a47ff24ba7f6b7b8575bc47`。本轮没有云请求、Provider 请求或照片外发；真实云、正式签名、授权图片评测、OEM、真人无障碍、生产卡复算与 cohort 仍缺，整体继续 `NO_GO`。

2026-07-29 Beta.69 可见且隐私安全的反馈学习（历史产品摘要，已被 Beta.70 取代）：反馈已在本机改变 topic affinity 与补卡排序，但旧界面只说“已记录”，设置页也只展示三个显式兴趣。现在“有意思 / 没意思”会在卡片上持续解释后续效果，设置页把“更常留意 / 减少推荐”与显式兴趣分开；摘要只读取仍保留、已排期卡片的受控物件名，不显示照片，也不重新暴露“识错了”或“太私人”卡片。标准与 1.6× API 34 专项通过；Data 88/88、最终干净 App 29/29，Android JVM 258/258（73/109/76），双模块双变体 Lint、Debug/App 测试 APK/R8 unsigned Release、源码门禁、候选复验和差异检查通过。全套回归发现的旧无障碍节点与残留 UploadWorker 已通过节点重取和唯一 Work 隔离修正，目标回归 4/4 后再完成 29/29。审计 `.tooling/beta69-visible-feedback-learning/audit.json` SHA-256 `422900db08d2745784086af88680a7caa0c051e45eda8fe3ac3b36834f85edde`；候选 `.tooling/release-candidate/beta69-visible-feedback-learning.json` 为 `LOCAL_CANDIDATE_GO`、`releaseEvidence=false`，SHA-256 `2171691cde3101f945361d122e17a5df15c332f7a48afa81f7af2b6c45f2c4e6`。真实云、正式签名、授权图片评测、OEM、真人 TalkBack、生产卡复算与 cohort 仍缺，整体继续 `NO_GO`。

2026-07-29 Beta.68 无可靠命中后的可恢复引导（历史产品摘要，已被 Beta.69 取代）：显式导入的真实冷启动路径审查发现，用户主动分析照片却没有可靠事实命中时，旧界面只说明原因并给出“换一张照片”，没有教会用户如何提高下一次成功率。现在 `NO_MATCH` 与技术失败分层：它保留“不为出卡而猜测”的可靠性承诺，用杯子、雨伞、扫帚、充电线和主体、光线、文字、人脸四项标准指导下一张，并把主操作具体化为“换一张日常物品照片”；网络/服务失败和权限失效仍保留各自恢复路径。第一次 1.6× 视觉验收发现标题末字孤行，收紧标题后重跑标准/大字，两档均完整可达。API 34 Data 88/88、App 29/29；Android JVM 256/256（73/109/74），双模块双变体 Lint、Debug/App 测试 APK/R8 unsigned Release、源码门禁、候选复验和差异检查通过。审计 `.tooling/beta68-no-match-recovery/audit.json` SHA-256 `003368b49ac19ee3b6f2c2e04cbf3e4224c207c025755bfedbf5f149267f9bf0`；候选 `.tooling/release-candidate/beta68-no-match-recovery.json` 为 `LOCAL_CANDIDATE_GO`、`releaseEvidence=false`，SHA-256 `02daef43aa8430f1cc292ad8ac1e7cc27719adea0395ade136713e90c662c274`。真实云、正式签名、授权图片评测、OEM、真人 TalkBack、生产卡复算与 cohort 仍缺，整体继续 `NO_GO`。

2026-07-29 Beta.67 来源优先的组件转化（历史产品摘要，已被 Beta.68 取代）：当前渲染审查确认 2×2/4×2 组件本身已经具备清楚的照片与知识层级，但 App 内安装推广位于正文和识别／来源之间，打断事实与证据。现在卡片按正文 → 识别对象 → 推送原因 → 来源 → “每天在桌面看一张”组件邀请的顺序阅读；API 34 几何回归证明来源先于邀请，邀请仍在首屏内，系统 Pin 流程继续成功。专项 8/8、清空 AVD 后 Data 88/88、App 29/29；Android JVM 255/255（73/109/73）、双模块双变体 Lint、Debug/R8 unsigned Release、源码门禁、候选复验和差异检查通过。审计 `.tooling/beta67-source-first-widget-prompt/audit.json` SHA-256 `fdd8d45e91f114868b77e3325845583c01899cf3032f4600624bf83404c462a8`；候选 `.tooling/release-candidate/beta67-source-first-widget-prompt.json` 为 `LOCAL_CANDIDATE_GO`、`releaseEvidence=false`，SHA-256 `30f6d4fc2f5f7226a5b19129fe373c73c0bf395ae7ad4d50bbb75e36f28a0ef5`。真实云、正式签名、授权图片评测、OEM、真人 TalkBack、生产卡复算与 cohort 仍缺，整体继续 `NO_GO`。

2026-07-29 Beta.66 模式切换与后台行为一致（当前最新产品摘要）：Beta.65 已能解释“提前备好一周”和“当天只理解一张”，但审计发现切换偏好时已运行的旧自动链可能继续按旧模式处理后续候选。现在模式变更由 Domain UseCase 统一执行：持久化新模式、等待取消全部自动 WorkManager 链、按现有照片权限重排；Privacy/Upload Worker 在每个候选前再次核对模式，陈旧计划立即停止，手动选择/分享导入保持独立。首次设置设备回归还发现注入 UseCase 与 ViewModel 方法同名导致递归调用被操作门禁抑制，已重命名并用同一测试证明选中态、偏好和 Activity 重建一致。WorkManager 专项 1/1、设置/TalkBack 1/1、Data 设备 88/88、清空冷 AVD App 29/29；Android JVM 255/255（73/109/73）、双模块双变体 Lint、Debug/R8 unsigned Release、源码门禁与差异检查通过。审计 `.tooling/beta66-mode-switch-consistency/audit.json` SHA-256 `08d24b6a514655c2f3bdc1623d7c92c03534dc04adb029340ef4034d616f89b7`；候选 `.tooling/release-candidate/beta66-mode-switch-consistency.json` 为 `LOCAL_CANDIDATE_GO`、`releaseEvidence=false`，SHA-256 `a38bcb0aee253730b2d32f8bcf3198650ee702223c9b76e988ed66c73a0ddf64`。真实云、正式签名、授权图片评测、OEM、真人 TalkBack、生产卡复算与 cohort 仍缺，整体继续 `NO_GO`。

2026-07-29 Beta.65 自动发现两种模式（历史产品摘要，已被 Beta.66 取代）：首次体验第三页已改为“选择你的开始方式”，并在“自动发现”卡片内提供“提前备好一周（推荐）”与“当天只理解一张”两种单选；前者准备 7–14 张卡片支持离线组件，后者按中国自然日最多上传分析 1 张且可靠知识未命中时沿用上一张。“仅选择照片”保持独立入口，设置、状态摘要、切换反馈和 TalkBack 使用同一口径。Android 14/API 34 首次体验 2/2、目标竞态 2/2、冷启动 App 全量 29/29；Android JVM 249/249（69/107/73）、后端 132/132、TypeScript、Data/App 双变体 Lint、Debug/R8 unsigned Release 和本地门禁全部通过。Debug / unsigned Release / App 测试 APK SHA-256 为 `b51777ea70856e42c103e4b2d9b325a80f89d26253206060385d2046947c15b4` / `834232d3918483767804cfd2f7e8c48ad89b20e2ba98f1b07b730cd248d6c6db` / `efbfeb509657d1566a1792f3e6b36135222dce95f5b4e157120891236b872877`。审计 `.tooling/beta65-onboarding-mode-clarity/audit.json` SHA-256 `7a99782f8e155d4230e257f9eb5a380da90ab6f6c57ff467fdd28a24ec1fd542`；候选 `.tooling/release-candidate/beta65-onboarding-mode-clarity.json` 为 `LOCAL_CANDIDATE_GO`、`releaseEvidence=false`，SHA-256 `e26abddbbc306390f77e0c78ae0b2705dd522f718f85f30ef0c967e1e84176e9`。真实云、正式签名、授权图片评测、OEM、真人 TalkBack、生产卡复算与 cohort 仍缺，整体继续 `NO_GO`。

2026-07-28 Beta.64 显式导入单一进度焦点（当前最新产品摘要）：真实截图复核发现首张照片等待页同时出现顶栏转圈和三阶段卡片转圈，两个“正在处理”信号竞争注意力。现在只在每日页确实显示导入进度卡时抑制重复顶栏分析转圈；卡片内与阶段绑定的活动指示保留，切到其他分区仍显示顶栏分析状态，用户删除等原子操作也继续拥有更高优先级。Android 14/API 34 专项 1/1 且 FILTERING/SYNCING 无障碍树都精确为 1 个 `ProgressBar`，完整 App 29/29；Android JVM 249/249（69/107/73）、Data/App Debug/Release Lint、Debug/R8 unsigned Release、源码门禁全部通过。审计 `.tooling/beta64-single-import-progress-indicator/audit.json` SHA-256 `c1a02dae5a11ebbb5e87aeddb1e30e8fd0be4f4d52c7f7a7448b5470bfad5dea`；新候选 `.tooling/release-candidate/beta64-single-import-progress-indicator.json` 为 `LOCAL_CANDIDATE_GO`、`releaseEvidence=false`，SHA-256 `3787ae8536eaadd258cfaf89570e4ec51b393396cab9c8713d871f8339622833`。真实云、正式签名、授权图片评测、OEM、真人 TalkBack、生产卡复算与 cohort 仍缺，整体继续 `NO_GO`。

2026-07-28 阿里云生产部署预检（当前外部边界）：新增 secret-safe 聚合预检，覆盖 Serverless Devs access、部署/迁移/构建工具、FC/RDS/OSS/VPC/HTTPS、固定北京 Qwen、知识库摘要、成本熔断与 OCI 摘要绑定；自测拒绝 8 类绕过并确认输出不含合成 API Key 或数据库密码。当前机器实测为 `NO_GO`：`psql=1`，Serverless Devs/access/Docker/不可变镜像均为 0，13 个真实部署变量未注入；报告 `.tooling/cloud-deployment-preflight-2026-07-28.json` 明确 `releaseEvidence=false`、`cloudStateObserved=false`。百炼 CSV 只能用于模型调用，不能替代 RAM/Serverless Devs 身份。此结果让下一次云交接可复现，但不等于 ACR/FC/RDS/OSS/HTTPS 已部署，整体仍为 `NO_GO`。

2026-07-28 Beta.64 首张照片三阶段进度与真实云边界（当前最新摘要）：显式导入等待卡新增“第 1 / 3 步 · 准备照片”“第 2 / 3 步 · 本机隐私筛选”“第 3 / 3 步 · 识别并匹配知识”和三段进度条；暂停、云端删除未完成和重试不显示虚假阶段。`SYNCING` 明确候选图先去除位置等元数据再用于识别；失败与重试文案不再说“照片仍安全保留在本机”，而是准确区分本机重试副本与“如已进入云端，临时图片最长保留 24 小时”。干净 Android 14/API 34 AVD 上阶段专项 1/1、App 全量 29/29；Android JVM 248/248、后端 132/132、TypeScript check/build、双模块双 Lint、Debug/R8 unsigned Release 及知识/来源/API/供应链/部署/预算/源码门禁全部通过。审计 `.tooling/beta64-truthful-import-progress/audit.json` SHA-256 `3e7f1b36e3428fc5ee1c77b39d1ca76faeea042e561fb17702398716cc258b27`；当前候选 `.tooling/release-candidate/beta64-truthful-import-progress.json` 为 `LOCAL_CANDIDATE_GO`、`releaseEvidence=false`，SHA-256 `eaace1b91ec69589c183d9f85215dc0d0dfc2b52a25363e8f84c4ac78f148c37`。一次旧快照 `offline` 和一次 ADB 无响应属于已保留的模拟器基础设施记录，清空 AVD 后专项、截图采集和全量回归均通过；它们不替代真实云、正式签名、授权图片、OEM、真人 TalkBack、生产卡复算或 cohort，因此整体仍为 `NO_GO`。

2026-07-28 Beta.64 首张卡片到桌面组件闭环（当前最新摘要）：最近从系统选择器或分享生成的第一张知识卡，现在在核心知识之后直接出现“把这张知识放到桌面 / 添加到桌面”，不再要求用户先返回普通每日流；“有新卡时会自动更新”替代无法保证的每日供给承诺，已安装组件时不重复展示。失败重试改为同步发布新状态并创建新的唯一 Work，修复旧 `FAILED` 回调覆盖新 `QUEUED` 的竞态；Room 多实例失效和暂停屏障持久状态同步也已闭环。API 34 App 28/28、Data 87/87、Android JVM 245/245（69/107/69）、后端 132/132、TypeScript、TCP E2E、双模块双 Lint、Debug/R8 Release 与全部本地门禁通过。审计 `.tooling/beta64-first-card-widget-loop/audit.json` SHA-256 `4fa09a130fa9ccdb86934f697a2bb9124f22132474d12b63cb71beba7285777c`；当前候选 `.tooling/release-candidate/beta64-first-card-widget-loop.json` 为 `LOCAL_CANDIDATE_GO`、`releaseEvidence=false`，SHA-256 `1fdb01abe3745fd0dc182270d7cbb1a0b1d76863eb810bef24f8f19d984a7fa6`。真实云、正式签名、授权图片评测、OEM、真人 TalkBack、真实生产卡复算和 cohort 未完成，整体仍为 `NO_GO`。

2026-07-28 Beta.64 全量自动卡片审核（当前最新摘要）：用户确认的全量 AI 审核已从知识目录延伸到生成卡发布门禁。554 条一般知识继续绑定固定 Qwen + 生产 `cip` 批准签注；生成卡正文和来源必须与目录精确一致，标题只允许三种稳定模板或低置信度“这可能是”，个人上下文只允许授权照片通用句或有效拍摄日期。真实 Beta 仍需从 PostgreSQL 导出 200–500 张脱敏卡片，但现在由 `derived-ai-reviewed-card-v1` 自动复算，人工逐卡审批不再是发布前置；health/safety 混入、政治审核理由异常、缺失审核凭证及任一字段漂移均失败关闭。编译器 14 类、Beta 门禁 32 类对抗自测，后端 132/132、TypeScript、证据装配/签名及源码/API/供应链/知识/部署/预算门禁全部通过。Android 字节未变，复用此前 28/28 设备和 244/244 JVM 证据。审计 `.tooling/beta64-automatic-card-audit/audit.json` SHA-256 `3df27637b6cfd0185ae8d5419076b67dfc6cc3cc39a905c4f850469a952fafb3`；当前候选 `.tooling/release-candidate/beta64-automatic-card-audit.json` 为 `LOCAL_CANDIDATE_GO`、`releaseEvidence=false`，SHA-256 `88a7e96507a9444af7949b1a5a5023036ced95936b8adb2cdbe4b9f07aa3a01f`。真实云、正式签名、授权图片评测、OEM、真人 TalkBack、真实生产卡自动验证和 cohort 未完成，整体仍为 `NO_GO`。

2026-07-28 Beta.64 桌面组件来源入口（当前最新摘要）：4×2 组件在只有当前卡或到达卡池末尾时，不再用“暂无更多卡片”形成死尾部，改为绿色行动“查看照片与来源 →”；来源发布方由 10sp 提高到 11sp，缓存过期提示缩为“缓存用完 · 点按更新”，可换卡状态与每日两次原子配额不变。API 34 Pixel Launcher 使用真实右侧缩放手柄完成 2×2 → 4×2，组件宽 966/1080px，正文底部 457px，来源行动 738–780px 且完整位于组件底部 811px 内。专项 1/1、最终 App 设备全量 28/28、Android JVM 244/244（69/106/69）、后端 132/132、TypeScript、双 Lint、Debug/R8 Release、源码/API/供应链/知识/部署/预算门禁均通过。Debug / 未签名 Release / 测试 APK SHA-256 为 `7bea036ff6dbd48116f770b462d831f4fa654b520bdefb5ac24c09e347c86017` / `ec749b99e437bde4fdd55bbfe768bd21ca00386e2bec42878ea1fd683366e0b8` / `a0f08255d959c59865476c40cb9e321ef8713a3b945c48626a3bca16350b754a`。审计 `.tooling/beta64-widget-editorial/audit.json` SHA-256 `1c7aeb5185e48e092236fad852f1a6a8cbf0a38379f67e4753a49aabd5ceac3a`、`releaseEvidence=false`；当前不可变候选 `.tooling/release-candidate/beta64-widget-editorial.json` 已按 Release + Debug APK 双绑定复验为 `LOCAL_CANDIDATE_GO`，SHA-256 `a4841bed1bd67d07646b117e4e45b2baf12e4496d2de0e30aed84d55b2540b07`。用户再次确认全量审核，一般知识仍固定 Qwen + 生产 `cip`、不确定即拒绝，health/safety 保持 draft；本轮未新增云端发送。真实云、正式签名、OEM、真人 TalkBack、200 卡抽检和 cohort 未完成，整体仍为 `NO_GO`。

2026-07-28 Beta.64 知识卡编辑区收敛（历史摘要，已被桌面组件来源入口候选取代）：知识卡下半部不再连续嵌套识别胶囊、灰色来源卡和灰色反馈卡。识别对象改为与正文对齐的轻量元数据；推送原因、可点击来源、收藏/提醒和反馈成为同一白色知识卡里的顺序阅读区，只由两个 `outlineVariant` 分隔线划分阶段；首次组件转化仍是唯一有意强调的内嵌行动区。API 34 真实 Compose 布局测量为正文、识别、推送原因、反馈标题左缘全部 89px，三个对齐差均为 0px；专项 1/1、相关知识卡/组件 3/3、最终干净 App 设备全量 28/28、Android JVM 244/244（69/106/69）、双 Lint、Debug/R8 Release 均通过。Debug / 未签名 Release / 测试 APK SHA-256 为 `b0e1826c0e06f04e099fc53d33b5bea5435037ae0fa8a33d2780adee4511923e` / `85f5cb0fdc1f693f8d03f19b1bcda8765cd3c45626a0cab658b698b24ea950e5` / `9b55298d0fca1e699100ce31367e7252040dd54edf2eaf120acf7d3543451910`。审计 `.tooling/beta64-editorial-sections/audit.json` SHA-256 `390ee527f303f15e2cf1cf9d925f43a0e433d1335ebfeaabd4e1fdc9b89de4de`、`releaseEvidence=false`；候选 `.tooling/release-candidate/beta64-editorial-sections.json` 当时按 Release + Debug APK 双绑定复验为 `LOCAL_CANDIDATE_GO`，SHA-256 `be5fadf15cf6c0badc11d570712c926b4fe8c1ed44f4c180b48a3f61317e37fa`。全量审核策略不变且本轮未新增云端发送；真实云、正式签名、OEM、真人 TalkBack、200 卡抽检和 cohort 未完成，整体仍为 `NO_GO`。

2026-07-28 Beta.64 编辑型首页页签（历史摘要，已被知识卡编辑区候选取代）：顶部“每日 / 收藏 N / 设置”从三个描边胶囊改为等宽文字页签，当前页用 28×3dp 底部主色短线标记，未选页退为次级文字色；48dp 点击高度、Tab 语义、选中态、动态收藏数与三页滚动隔离不变。API 34 视觉复核显示导航不再与每日知识卡和组件入口争夺主操作层级；窗口、正文 869–995px、组件入口 1063–1116px 与 179px 缺图提示坐标和上一候选逐字节一致。设置导航专项 1/1、系统组件 Pin 复验 1/1；第一次干净全量 26/27 出现一次动态“收藏 1”反向滚动等待超时，同方法隔离复验 1/1，第二次干净 App 设备全量 27/27。Android JVM 244/244（69/106/69）、Debug/Release Lint、Debug/R8 Release 均通过。Debug / 未签名 Release / 测试 APK SHA-256 为 `05f4e9b43d297ce3713a6aeb92f60dbee2e4a69c2e4d6354366c2a34059cc898` / `397cbacf8a57bdcb1b0fac06ed7a62467a13ec940b2c7a7fc4bdefa7b012484c` / `885f2294bbab892f7d6137375f0f081d3c2fc8498f8721c6e9898e31fa23c890`。审计 `.tooling/beta64-editorial-tabs/audit.json` SHA-256 `c1bfc72457b9d5ff331dfe7ad6e1b547f21c8b6afb5e6a73130fa5fa16e9a0ae`、`releaseEvidence=false`；候选 `.tooling/release-candidate/beta64-editorial-tabs.json` 当时复验为 `LOCAL_CANDIDATE_GO`，SHA-256 `8a29425685a985ca0d166fbb43222edbab8798e0362014bed5d65c35c26a7be1`。全量审核策略不变，本轮未新增云端发送；真实云、正式签名、OEM、真人 TalkBack、200 卡抽检和 cohort 未完成，整体仍为 `NO_GO`。

2026-07-28 Beta.64 重访首页紧凑化与全量审核确认（当前最新摘要）：首页顶栏收为单行品牌栏，移除已在首次体验说明过的价值承诺；每日主卡已有“今日一知”语义，因此不再重复显示“今天”分区标题，三段导航仍保持 48dp 点击目标。API 34 的 2400px 窗口中，知识正文顶部由上一候选的 1114px 前移至 869px，组件入口由 1308px 前移至 1063px；专项 3/3、冷启动物品提醒复验 1/1、最终干净 App 设备全量 27/27、Android JVM 244/244（69/106/69）、后端 132/132、TypeScript、双 Lint、Debug/R8 Release 均通过。一次被手动中止的全量测试留下 UTP 卸载竞态，冷启动后未改产品代码即通过隔离与全量复验。Debug / 未签名 Release / 测试 APK SHA-256 为 `037e2c4eb93629f906b213e2678c64b26a10a5e6ebcb374fb2daae3baba6b978` / `17dae9b308d64621851f278fb5fa8c18af360ef6515f34889eb1cf61b2676f14` / `456ac2035e6abb2107dd013d8a9f9de9264221230eb18e3825452c18f1b7d43a`。审计 `.tooling/beta64-compact-home/audit.json` SHA-256 `718a4662c2e6a8a0013a63507bfa3c2f4efbebc8bde7a306990f66a7e3407728`、`releaseEvidence=false`；当前不可变候选 `.tooling/release-candidate/beta64-compact-home.json` 已复验为 `LOCAL_CANDIDATE_GO`，SHA-256 `495cb3fdc65ea71e37acea79975433c0bfdbf4945130117804df788d5575e5d0`。用户确认继续执行全量审核：一般知识固定 Qwen + 生产 `cip`、主要风险全覆盖且不确定即拒绝，health/safety 保持 draft；本轮未新增云端发送。真实云、正式签名、OEM、真人 TalkBack、200 卡抽检和 cohort 仍缺失，整体继续 `NO_GO`。

2026-07-28 Beta.64 缺图状态不再挤走知识内容（当前最新 Android 产品体验摘要）：卡片缩略图现在区分加载中、可用和不可用三种状态；正常照片继续保留 190dp 大图，不可读照片收为 68dp 诚实提示，并以单一容器语义供 TalkBack 读取。API 34 的 2400px 窗口中，缺图容器实测 179px，正文位于 1114–1240px，组件入口位于 1308–1361px，来源、收藏和提醒也进入同一屏；生成 JPEG 的专项证明可用照片高度仍大于窗口六分之一。专项 2/2、当前字节组件复验 1/1、最终干净 App 设备全量 27/27、Android JVM 244/244（69/106/69）、双 Lint、Debug/R8 Release 均通过。第一次全量设备回归为 26/27，唯一失败来自依赖继承字体/滚动状态的间接正文位置断言；改为直接测量缺图容器后专项和全量均通过。Debug / 未签名 Release / 测试 APK SHA-256 为 `978ab174a8cfa8fe0463b83c92a108a46350bcb6bf248d12a6db86c2f50309ed` / `62a6c7b5347300251dad1b273f466d200400c0da273f66f303e081759a3d2c6c` / `5ec588e50160030c3de9a01ebc9abb62882db8b6be7f80dacd4973c576866e90`。私有审计 `.tooling/beta64-compact-missing-photo/audit.json` SHA-256 `4592b5bcc117ccfbe6379034de10e66ba1f39172251463fe4ca34f2c2806374b`、`releaseEvidence=false`；当前不可变候选 `.tooling/release-candidate/beta64-compact-missing-photo.json` 已复验为 `LOCAL_CANDIDATE_GO`，SHA-256 `a66d055d76645fbaaca5a456c93342b6bf03fdbc36e6a88c4d7cffb69ed5d6e1`。真实云、正式签名、OEM、真人 TalkBack、200 卡抽检和 cohort 仍缺失，整体继续 `NO_GO`。

2026-07-28 Beta.64 价值优先组件入口与全量 AI 审核确认（当前最新产品与审核策略摘要）：首次每日卡的组件入口已从完整详情底部前移到核心知识正文之后，用户仍先看到照片、对象标题和知识正文，但不再需要越过来源、收藏、提醒和四类反馈才找到产品主转化。API 34 的 2400 px 高窗口中，正文底部为 1560 px，入口为 1628–1681 px，无需滚动；系统 Pin 完成后“为什么推给你”仍可滚动到达。组件专项 1/1、第二次干净 App 设备全量 27/27、Android JVM 244/244（69/106/69）、双 Lint、Debug/R8 Release 均通过；第一次设备全量中的设置点击瞬态失败已独立复验并由干净全量覆盖。Debug / 未签名 Release / 测试 APK SHA-256 为 `8e4f9b97b7444dd9496d46212806666897fb06030d2c36fd5fd7436b52bfbef6` / `cd8a478a5c97715b3b266c7a6429e732ae35f5b77a96ec120e992c79c322b73f` / `58e633c50b724892a5dd5b38b7be270dbda21e3abcd8fc23d305476966568e30`；审计 `.tooling/beta64-value-first-widget/audit.json` SHA-256 `20a3555a96487e2c857951e857f647a176462490cd9a6c8945b594ed0ea494b3`、`releaseEvidence=false`。新不可变候选 `.tooling/release-candidate/beta64-value-first-widget.json` 已复验为 `LOCAL_CANDIDATE_GO`，SHA-256 `04afd7cbfbce9cd25e6505185b1ca18557a51f2a6676744f0fd25b7eab3512ff`。用户已确认全量审核；执行方式为固定 Qwen + 生产 `cip` 自动审核所有一般知识，覆盖涉政及其他主要风险，不确定即拒绝，高风险 health/safety 首版不自动发布，人工只做异常处理与抽检。真实云、正式签名、OEM、真人 TalkBack、200 卡抽检和 cohort 仍缺失，整体继续 `NO_GO`。

2026-07-28 Beta.64 知识卡打开即见内容（当前最新 Android 产品体验摘要）：组件、分享导入、收藏和往日记录的精准入口已从“大说明卡 + 全宽返回按钮”统一压缩为单行上下文栏，移除会把真实卡片推到首屏下方的教学文案。API 34 真实 Compose 布局中，入口标签与返回动作中心线差 0 px，知识卡标题顶部为 1196/2400 px（49.83% 屏高），旧说明节点不存在；专项 1/1、App 设备全量 27/27、Android JVM 244/244、双 Lint、Debug/R8 Release 均通过。Debug / 未签名 Release / 测试 APK SHA-256 为 `2f984cddd85c60dd88e1ab254b527632dfa296adcd4af9e67e1a6f3adc76dbb7` / `26f25160adf0a82af8613961da20de458f76409deab5d4a3117f8f388819651f` / `75924bfe1ab9f5eb8db3606d7c34f81fd030439a5b036020d7721adfc53a311d`。私有审计 `.tooling/beta64-compact-card-entry/audit.json` SHA-256 `e2a633c5e2e6df7e52218bce279e5d5abad719ba299edb8c9944953c2bcc0818`、`releaseEvidence=false`。新不可变候选 `.tooling/release-candidate/beta64-compact-entry.json` 已复验 `LOCAL_CANDIDATE_GO`，SHA-256 `3e0c9d0d7830d9aed104be9d826c448d8261feb71f9f91b0c630b62857f9207f`；模拟器证据不替代正式签名、真实云、OEM、真人 TalkBack、卡片抽检或 cohort，整体仍为 `NO_GO`。

2026-07-28 Beta.64 对象裁图阶段发布候选（历史发布工程摘要，已被紧凑入口候选取代）：新增不可覆盖、可复验的本地发布候选清单，把后端 Release identity、14 个 PostgreSQL 迁移及逐文件 SHA、Beta.64 知识目录、OpenAPI、Room 14、Docker/FC 模板、生成器自身与 Gradle 未签名 Release APK 绑定为同一候选。`.tooling/release-candidate/beta64-final.json` 当时为 `LOCAL_CANDIDATE_GO`、`releaseEvidence=false`；后端 SHA `50323c815aa3abbd4a01e5479ff7555d10edc10a795caf810d6b3245422831b8`，知识 SHA `ef26febc1520d9b46e74dd34a985ed2d2e270cd857dea21456f5e93a8e88a923`，未签名 Release APK SHA `6039743e95a4e23c4a65c960202636e2296980a573b209258d1d8f3f7da9cd6a`。清单重复写入被 `EEXIST` 拒绝，生成器变化后的旧清单被 stale gate 拒绝，当前字节复验通过；5 个不安全反例、部署模板、运行预算、源码守卫与差异检查均通过。机器固定顺序为迁移 014 → 后端镜像 → 云健康/护栏验证 → Android 签名分发；失败时保留 nullable additive 的迁移 014 并回退旧镜像。真实 ACR/FC/RDS/OSS/HTTPS、独立部署回执、正式签名、OEM 和 cohort 仍未产生，整体 Beta 保持 `NO_GO`。

2026-07-28 Beta.64 一般知识审核全量收口（当前最新内容运营权威摘要）：Beta.63 仅剩的餐叉与 USB 数据线两条超长正文已分别压缩为 78/77 字，移除旧 `format_invalid` 拒绝签注后，只将两条修订正文、主题名和公开来源元数据发送至固定 `qwen3.6-flash-2026-04-16` 与生产 `cip` 护栏重审。本轮 1 次有护栏请求返回 2/2 `approved / safe_general`、0 rejected，499 input / 93 output tokens；70 条 health/safety 未发送且继续保持 draft。目录已原子升级为 `2026-07-28-beta.64`，SHA-256 `ef26febc1520d9b46e74dd34a985ed2d2e270cd857dea21456f5e93a8e88a923`；554/554 条一般知识均 approved、verified、AI-reviewed，readiness 为 183/200 ready topic、0 blocker。私有增量报告 `.tooling/ai-knowledge-review/final-2-2026-07-28.json` SHA-256 为 `c4054f25997ec47cc525a5f17a6573ac249b8820653cb2541c452649086fded8`、`releaseEvidence=false`。后端 132/132、Android JVM 241/241（66/106/69）、双 Lint、Debug/R8 Release、API 契约与源码守卫均通过；当前 Debug / 未签名 Release APK SHA-256 为 `abc306225577b7c836f80917da15226b5253c93aef6ec4321a6f63b1b78c44ba` / `ecbf67c21c433535d9c5917f1bcff8824860e7762e5cdb3a6ac2afc7159370c1`。一般知识内容审核已无待处理项；真实托管云、正式签名、国产 OEM、真人 TalkBack、200 卡抽检和 cohort 仍使整体 Beta 保持 `NO_GO`。

2026-07-28 Beta.63 Android → Qwen 真实产品闭环（当前最新跨端权威摘要）：用户此前明确授权的项目无人物扫帚图经 Android 14 系统 Photo Picker、端侧隐私/质量筛选、压缩临时上传、真实 Qwen 视觉识别、Beta.63 事实检索、Room、Compose 与 Glance 完整跑通。端侧真实标签只有 `Room / Chair / Pattern`，Qwen 在 `cip` 输入/输出护栏下返回 `broom / 扫帚 / 0.95`，卡片发布已批准一般事实 `broom-draft-004` 与 Google Patents 来源；临时对象归零，跨日后 4×2 仍显示来源和缓存提示并精准打开同一“昨日一知”，crash buffer 为空。实跑同时识别并修复两项假成功/可信度问题：`Room` 不再通过 substring 误命中 `broom`，4×2 缓存耗尽状态不再替换知识来源。后端 132/132、Android JVM 241/241（Domain 66、Data 106、App 69）、API 34 组件专项 2/2，知识 readiness、API 契约与源码守卫均为 GO，R8 Release 重建成功。Debug / 未签名 Release APK SHA-256 为 `a87ce1d09106e80aa140bb0fffa99adee60bac13f47f619143125cea065b1184` / `ecbf67c21c433535d9c5917f1bcff8824860e7762e5cdb3a6ac2afc7159370c1`；私有审计 `.tooling/beta63-photo-to-card/audit.json` 明确 `releaseEvidence=false`。该结果不替代真实托管 PostgreSQL/OSS/HTTPS、正式签名、国产 OEM 实体机、真人 TalkBack、200 卡抽检和 10–50 人 cohort，整体 Beta 仍为 `NO_GO`。

2026-07-27 一般知识 AI-only 全量审核（当前最新内容运营权威摘要）：用户已明确授权将 554 条一般知识文本、主题名和公开来源元数据发送至百炼。固定 `qwen3.6-flash-2026-04-16` 最终批准 552 条；两条 84/87 字正文由本地 28–80 字硬门禁改为 `format_invalid` 拒绝。70 条 health/safety 未发送且不发布，旧高风险批准状态已降回 draft。目录已原子写入 `2026-07-27-beta.63`，SHA-256 `01c6e1d1eade84dac63363d24ba1624dc06b2b0f495321fa127d83cc0548d18d`；readiness 为 183/200 ready topic、552 verified fact、0 blocker。首轮全量结果因模型错批超长正文被最终门禁整体拒绝且未写目录；随后加入确定性格式拒绝和批次检查点，恢复过程中又拒绝一次 `decisions` 对象形态并加入有界格式重试。被接受审核轮记录 28 批、76977 input tokens、22746 output tokens、0 unguarded call；含丢弃尝试在内，已知百炼调用至少 57 次，账单是精确成本权威。后端 131 项测试通过、14 项 PostgreSQL 按环境跳过，知识、部署、API、证据和源码门禁均为 GO。人工工作台仅保留为可选纠错；内容门禁已闭环，但整体 Beta 仍受真实托管云、正式签名、国产 OEM、TalkBack 与 cohort 阻断。

2026-07-26 一般知识启动审核批次（当前最新内容运营权威摘要）：原审核队列为控制风险而把 health/safety 排在最前，当前默认 20 条批次因此只有 4 条 general，不能高效形成首批日常物件可用主题。审核模板与本地工作台现支持 `--risk general --whole-topics`：只选择全部待审核事实均为 general 的主题，并以“完整主题”为最小单位，不会为凑满数量拆开主题；筛选策略写入不可变工作台会话，恢复时按同一目录 SHA 复核。它不预选决定、不勾选来源、不写知识目录，也不放宽真人 checkpoint。当前 `2026-07-19-beta.62` 真实队列预检得到 19 条 / 6 个完整主题：空气净化器、闹钟、铝箔、背包、圆珠笔、台钳；待审核模板 `.tooling/knowledge-review-batches/beta62-general-starter-pending.json` SHA-256 为 `994df5e0e4ecaa5e459a68054231a53238001bbe7d1e5196cb4d0c390de82bcb`，19/19 均为 pending、0 来源勾选、0 自动批准。队列、模板、工作台、原子应用和 readiness 自测全部通过，源码守卫输出 `reviewLaunchBatch=1`。这把“先审核哪些内容”从高风险阻塞改成可形成 6 个候选 ready topic 的最短路径，但只有责任人逐条打开来源并签注后才会真正 ready；当前 0 ready topic 和 Beta `NO_GO` 结论不变。

2026-07-26 知识来源透明度与卡片可信度（当前最新 Android 权威摘要）：卡片详情不再只显示发布方名称，而是把每条已通过 URL 安全校验的来源呈现为“来源 · 发布方”与具体文章/页面标题；多来源按顺序编号，标题与发布方相同时去重，TalkBack 继续读取发布方和完整标题。该变化只在 App presentation 层实现，不改变事实匹配、来源安全校验、Room 或网络边界。API 34 标准字号和 1.6× 字体专项均通过，最终干净状态 App instrumentation 27/27；Android JVM 240/240（Domain 66、Data 106、App 68），App Debug/Release Lint、Debug、R8 Release、API 契约、差异检查和源码守卫 `sourceTransparency=1` 全部为 `GO`。Debug/未签名 Release/App 测试 APK SHA-256 为 `497394b26017fbb0d1d80e181db428c05cecd490300a3dec7b965b580085c6a4` / `a9336764f90203e13995b31b67b6e33ac1aff078513c1b4af53c5b6d7739b0bd` / `12a0ae2e0a50e3d2a6ee20006fcba9a047a587363a1267450536e79c9a28456f`。标准/大字视觉证据 SHA-256 为 `dea233db735261fa87bd8ed449aaa6284d31933649823c8e37695f4675f7d055` / `f646ee12e1aa88d124d864ccf6528a52d93fbd77b6c3eccfa25b900763416b1a`，均为本地模拟器工程证据。来源标题可见不等于事实已经获得真人语义审核；图片级 Qwen、真实托管云、正式签名、国产 OEM、真人审核和 cohort 仍未闭环，Beta 保持 `NO_GO`。
...
2026-07-26 上一轮完整图片 Provider 基线：使用用户下载的北京百炼工作空间按量付费 Key 和仓库自有、无人物的扫帚示例图，真实调用固定模型 `qwen3.6-flash-2026-04-16`。当时生产请求携带强制内容安全 Header 返回 `403 access_denied`；随后只用于定位的无内容安全文本探针返回 200，无内容安全视觉诊断识别为 `broom / 扫帚 / 0.98` 且无敏感标记。该 403 先被 `2026-07-26T13:10:57.561Z` 的生产 Header 文本探针 HTTP 200 所取代，随后又由下述获授权的生产护栏图片复验完整关闭；服务器运行时始终强制该 Header，不会为了跑通而降级。

2026-07-26 当前完整图片 Provider 复验：用户随后明确同意将项目中的无人物扫帚测试图发送至阿里云百炼。发送前人工复核画面只有室内墙边扫帚、家具和植物，无人物、面孔、证件、品牌或文字；WebP 转换后的 JPEG 由验证器在内存移除 JFIF/EXIF，实际发送净化字节 SHA-256 为 `8890d549cad0bcfb9952b9490284595a20ff2b2038fb70d36e32bf7b3dad1da2`、135067 bytes。`2026-07-26T15:34:42.988Z` 唯一一次生产视觉请求携带强制 `X-DashScope-DataInspection={"input":"cip","output":"cip"}`，固定 `qwen3.6-flash-2026-04-16` 于 5.802 秒返回 `broom / 扫帚 / 0.98 / sensitiveFlags=[]`，`providerGate=GO`；模型访问探针 0 次、无护栏视觉请求 0 次。报告以 `0600` 和 `wx` 写入忽略目录 `.tooling/qwen-provider-verification-2026-07-26-guarded.json`，SHA-256 为 `d1ca4e408e77c609bf1fa3cf94f8c1980e5b3f7f8ee91f98abb08af583c414fa`，写出前已拒绝 Key、完整端点和本地输入路径泄漏。该报告仍标记 `releaseEvidence=false`，证明当前 Qwen Provider 与生产内容安全护栏可工作，不替代真实托管云、OSS 删除、正式签名、OEM 实体机、真人知识审核、200 卡抽检或 cohort；Beta 保持 `NO_GO`。

Provider 验证器把失败路径收敛为一份机读报告：`providerGate=NO_GO`、`releaseEvidence=false`，显式记录上一轮实际 3 个请求，而不是只显示产品单卡的 1 次模型调用。原 JPEG 的 JFIF/EXIF 段在内存中移除，报告只保留净化字节 SHA-256 与大小；写出前扫描 API Key、完整工作空间端点、凭据路径和图片路径，命中即拒绝。报告以 `0600` 和 `wx` 写入，拒绝覆盖旧证据。上一轮真实报告位于忽略目录 `.tooling/qwen-provider-verification-2026-07-26.json`，SHA-256 为 `4cbb927227fad5dcaf3f362f3081f6db33e2cb421de03ab9a6ae528a29a1ff2b`；二次检查确认四类敏感值均不存在。它是已被当前 `providerGate=GO` 报告取代的历史诊断，不是发布证据。Beta 保持 `NO_GO`。
...
国产 OEM 七天矩阵、真人 `zh-CN` TalkBack、授权图片评测、200–500 张真实生产卡自动验证及真实 cohort 仍无证据，
...
| 要求 | 状态 | 证据或缺口 |
|---|---|---|
| 200 主题、每主题 3–5 条事实；首版只发布一般知识 | PROVEN（首版一般知识）/ DEFERRED（高风险） | Beta.64 保留 200 个受控主题和 624 条结构化事实：176 个主题各 3 条、24 个主题各 4 条。554 条 `general` 已全部经固定 Qwen 与生产 `cip` 护栏审核为 approved/verified，形成 183 个 ready topic；10 条 health 与 60 条 safety 继续保持 draft，不进入首版发布池。目录和运行时都拒绝未签注、超长或高风险草稿，人工工作台仅作为可选纠错与未来高风险流程，不再是一般知识逐条发布前置。 |
| 无可靠命中不发卡 | PROVEN | `needs_content` 分支与 draft-only 集成测试通过；已编译服务的真实 TCP 闭环也证明未知标签上传后只进入 `needs_content` 且不返回卡片 |
| 来源可访问性与语义审核 | PROVEN（身份/访问）/ PARTIAL（正文语义） | 静态门禁覆盖 531 个唯一公共 HTTPS 来源且无悬空引用；联网证据曾验证全部编辑来源可访问，解析仍经过 HTTPS、公网地址和重定向钉扎。一般知识 AI 审核会检查来源标题、发布方与主题方向，但明确没有打开网页正文，因此不把它冒充逐页语义核验。首版按用户确认的 AI-only 策略发布一般知识；真实 Beta 需对 200–500 张生产卡自动复算目录事实、来源集合、审核签注和展示策略。人工来源核对仅作可选质量调查，不授予发布权限。 |
| 模型不能编造 fact/source ID | PROVEN | 视觉模型不再接收或返回事实/来源；服务端确定性选择 approved fact 并直接写入正文、`factId` 与来源。标题也由服务端安全模板生成，因此模型没有改写或伪造这些字段的接口 |
| 健康/安全双权威来源 | DEFERRED（首版不发布） | Catalog 校验会拒绝不足两个权威来源的高风险 approved fact；当前 70 条 health/safety 全部为 draft，运行时不可选。未来若开启该类别，必须另建更严格的责任审核流程，不能复用一般知识 AI-only 放行。 |
| PostgreSQL 持久化 | PROVEN（本地）/ EXTERNAL（云） | 隔离的 PostgreSQL 17.10 三次走完全部 15 个迁移（含编译服务启动迁移）并通过 17 项真实集成测试；迁移 10 写入运行中后端 Release SHA-256，迁移 11 建立最小隐私删除回执，迁移 12 提供公平有界的对象删除重试，迁移 13 回填独立识别对象名，迁移 14 增加可空对象框，迁移 15 持久化反馈实际贡献以支持饱和权重精确撤销。4 个独立连接池验证预算、幂等、单次上传、租约恢复、删除/重装、偏好、授权评测、并发注册、卡片排期及并发隐私反馈事务；同一编译服务的完整 TCP 产品闭环真实经过 PostgreSQL，结束后进程已确认停止。托管实例、监控与故障注入仍未执行 |
...
1. 300–500 张明确授权评测图片，敏感漏传率 <1%，主要对象 Top-1 ≥90%。
2. 华为、小米、OPPO 或 vivo Android 实体机回归，包括完整/部分/拒绝权限、处理中撤权、后台限制、小组件和真人 TalkBack 听读。
3. 真实托管 PostgreSQL、OSS、Qwen、HTTPS 部署；验证内容安全、立即删除、一天生命周期、删除设备数据和故障恢复。
4. 使用独立私钥签出 Release APK；先 10 人 3 天，再扩到 20–50 人。
5. 至少 200 张真实生成卡通过自动派生审核，并达到组件添加率、7 日互动率、LIKE 率和首卡延迟目标。
...
2026-07-28 Beta.64 对象感知组件裁图（当前最新跨端权威摘要）：真实 Qwen 扫帚卡返回并落库归一化对象框 `{x:0.48,y:0.02,width:0.35,height:0.75}`。该框现从后端卡片、PostgreSQL 迁移 014、严格 OpenAPI、Android wire 校验与 Room 14 一路进入 Glance；新卡按对象中心裁图，旧卡/无效框安全回退居中裁图。Android 14 真实 Photo Picker 路径再次跑通，4×2 左图完整保留扫帚主体并精准回到同一卡。后端 132/132；隔离 PostgreSQL 14 个迁移、15/15 集成及 TCP E2E；Android JVM 244/244（69/106/69）、Room 迁移设备测试 12/12、组件专项 2/2；双 Lint、Debug/R8 Release、知识/API/源码门禁均通过。Debug / 未签名 Release APK SHA-256 为 `79249138638bc4eaac90f343d7cf0fd511325f0e1258c2a5b1a15dfdd0fc14ed` / `6039743e95a4e23c4a65c960202636e2296980a573b209258d1d8f3f7da9cd6a`。私有审计 `.tooling/beta64-object-crop/audit.json` 为本地工程证据；托管云、正式签名、OEM、真人 TalkBack、200 卡抽检与 cohort 仍未闭环，整体 Beta 继续 `NO_GO`。
