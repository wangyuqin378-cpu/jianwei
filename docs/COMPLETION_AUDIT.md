...
| 首轮 12 个候选、后台补足 7 张未来卡 | PROVEN | 排序限制、DEFERRED 队列、未来卡计数和单轮 24 个上传上限已实现。供给策略把“每天抽一张”的惊喜感与“先准备一批”的离线可靠性合并：自动发现按补卡当天的中国日历日期生成稳定种子，只在综合质量、时效、显式兴趣和反馈分差不超过 0.04 的最多 3 张候选内调整顺序；同一天重试/进程重启顺序不变，不同补卡日可产生不同首选，明显更差的照片不能靠随机上位。Picker/分享不传种子，始终保持质量与反馈排序。服务端不再按历史卡总数偏移日期，而在完成事务内选择中国日历今天起首个空位。PostgreSQL 四连接池 32 路并发证明日期连续唯一，删除中间卡后下一张补最早缺口；数据库 `date` 统一归一化为 ISO 字符串 |
