...
export function assessCloudDeploymentPreflight({
  ...
  const container = assessContainerDeploymentInputs(env);
  const blockers = [];
  ...
  blockers.push(...container.blockers);
  if (container.status !== "GO" && !tools.docker) {
...
async function runSelfTest() {
  ...
  process.stdout.write(
    `CLOUD_DEPLOYMENT_PREFLIGHT_SELF_TEST=GO synthetic=1 releaseEvidence=0 bypassesRejected=${cases.length + 3} ephemeralAccess=1 secretValuesPrinted=0\n`
  );
}
