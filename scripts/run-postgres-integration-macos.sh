...
if [[ "$SCHEMA_COUNT" != "17" ]]; then
  print -u2 "Expected seventeen applied PostgreSQL migrations, found: $SCHEMA_COUNT"
  exit 1
fi
...
if (total < 19 || failed !== 0 || pending !== 0) process.exit(1);
...
if (test.success !== true || test.numTotalTests < 19 || test.numFailedTests !== 0 || test.numPendingTests !== 0) {
...
const audit = {
  ...
  postgres: {
    ...
    managedDailySelectionCostReservation: true,
    failedJobRetryCostReservation: true,
    processStopped: true
  },
  tcpE2E: tcp.checks,
  artifacts: {
